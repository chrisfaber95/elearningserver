...
module.exports = (req, res) => {
   ...
   //   sqlquery += "union "
    //  var sqlquery = "SELECT * FROM `allTrainingen` WHERE `subonderdeel_id` = " + req.params.subId + " ;";
	  var sqlquery = "select `at`.`training_id`, `at`.`tImage`, `at`.`tName`, `at`.`tSlug`, `at`.`onderdeel_id`, `at`.`oName`, `at`.`subonderdeel_id`, `at`.`sName`, `at`.`sDifficulty`, `us`.`user_id`, `us`.`isVisible`, `us`.`difficulty`, `us`.`user_subonderdeel_id`, `us`.`isFinished` "
      sqlquery += "from `allTrainingen` as `at` "
      sqlquery += "right join `User_Subonderdeel` as `us` using (subonderdeel_id) "
      sqlquery += "WHERE (`user_id` = " + req.params.userId + " "
      sqlquery += "AND `subonderdeel_id` = " + req.params.subId + ") order by `subonderdeel_id` asc, `user_id` desc;"
